...
def discriminator_func():
    ...
    x = Conv2D(filters=16, kernel_size=5, strides=1,kernel_initializer=init, name='conv1')(img)
    x = SelfAttention(ch =16)(x)
    x = LeakyReLU()(x)
    x = Conv2DTranspose(filters=32, kernel_size=5, strides=1, padding='valid',kernel_initializer=init, name='primarycap_conv2')(x)
    ...
    x = Flatten()(x)
    uhat = Dense(100, kernel_initializer=init, bias_initializer='zeros', name='uhat_digitcaps')(x)
